...
/* ═══════════════════════════════════════════════════
   Sitemap Index — splits URLs into 3 child sitemaps
   for faster Google processing and cleaner GSC reports.

   Output at /sitemap.xml:
     → /sitemap/0.xml  (static + category pages)
     → /sitemap/1.xml  (calculator pages — canonical only)
     → /sitemap/2.xml  (blog posts)
   ═══════════════════════════════════════════════════ */

/** Tell Next.js to generate 3 child sitemaps */
...
export default async function sitemap(props: {
  ...
  if (id === '0') {
    ...
    // Category calculator list pages — high priority hub pages
  ...
  // ── Sitemap 1: Calculator pages (canonical slugs ONLY) ──
  if (id === '1') {
    // CRITICAL: Use getCanonicalCalculatorSlugs() NOT getAllCalculatorSlugs().
    // getAllCalculatorSlugs() includes ~140 redirect aliases that cause Google
    // to discover duplicate/redirect URLs, wasting crawl budget and inflating
    // the "Discovered – currently not indexed" count in GSC.
    return getCanonicalCalculatorSlugs().map((slug) => ({
      url: `${baseUrl}/${slug}`,
      lastModified: CALC_LAST_MODIFIED,
      changeFrequency: 'weekly' as const,
      priority: 0.9,
